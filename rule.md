...
The rule is a set of principles for structuring knowledge into brief wins. The biggest wins come first, and each breaks down into smaller wins, down to every detail that grounds them.
...
The rule is not something you move into. It compounds on prose, and prose is already everywhere, in every tool and every practice. The full experience needs technology that is not built: a medium open to everyone's technology and practices. But a harness opened on a folder, with full reach into everything in it, is the most open-ended way of using AI today, and with the rule it practises most of the medium now. A [proof of concept](poc/README.md) runs it with git and the harnesses that exist today.

## 3. What knowledge is made of

You now hold why the rule exists and what it is for. This section gives what knowledge is made of under it. The gradient is the thesis given shape. The brief is the part that carries one understanding. The ground is what keeps a brief true for whoever stands on it next.

### 3.1 The gradient

Reading is linear. A reader takes one thing after another, and prose can be laid in only one order. Knowledge is not linear: what matters most stands on many things. So the order has to be chosen, and two forces pull on it. Importance wants the biggest understanding first. Comprehension allows only what can be understood from what the reader already holds.
...
So the gradient is won one part at a time. Take any part and ask whether its most important understanding can be given without the rest. If it can, the rest goes beneath, and that break is a win in itself: the reader who wanted only the top now gets it for less, and the reader who wants the rest still finds it. The gradient is made of every such break, and it has as many steps as the knowledge yields.

Why before how follows. The understanding at the top is real but not yet practical, and each step down is taken knowing what it is for. This is the rule's thesis, and everything else in it serves it.

### 3.2 The brief

A part on the gradient is a brief: one piece of prose that gives one understanding, handed whole. It may make several points, but they add up to one thing. Everything the understanding needs is in the brief or pointed out of it, and that is what makes a brief a checkpoint.

The brief is as large as its understanding needs, and that is a limit, never a licence. The break of the gradient applies inside it: whatever the understanding can be given without goes beneath, into briefs of their own, to be entered wiser. Breaking down as far as the knowledge yields costs the reader nothing and hands them a checkpoint, so where a brief can be broken, it is.
...
Briefs beneath briefs make a holarchy, and the word is chosen because it says more than hierarchy does. A hierarchy says only how the parts rank, above and below. A holon is a whole that is also a part, and a holarchy is an order made of them, so the word says of every brief what the rule needs said: it stands whole and can be read alone, it is a part of the understanding above it and is itself broken down beneath, and it is free to relate beyond its place by linking.
...
The rule can sound as if it describes a finished thing. It never is: knowledge is built step by step on work in progress, and the rule is made for that. So a link needs no finished target, and two kinds of target are always allowed.
...
*Seen, in every piece written under the rule so far; not measured.*
...
So far the rule has mostly described knowledge as it stands. This section is about *how* it comes to stand, and the finding is that the making has the same shape as the made.
...
A working session is knowledge being made, so what the rule asks of a brief it asks of a session. The understanding comes in small before the whole. A thing is proposed before it is committed. A step is taken and left whole before the next. Where one stands is said before moving on.

Held this way the rule is not obeyed but embodied, in how one talks and works. And whoever holds it can hold the step for the other, the person for the agent and the agent for the person, wherever one of them runs ahead. This is where the rule stops being a shape and becomes a medium.

This section holds early experience, and what it finds will reach the whole rule once it is lived enough to lift. Where it comes from is more than method, and the author says so in [a personal note](study/10-comprehension/note.md), an ontological view of existence, the ground the rule emanated from.

### 5.1 What inference can do

A model infers; it does not comprehend. The rule does not say how to come closer, by memory, iteration or whatever comes next. It sits underneath, giving any such reasoning something to reason about other than the words, and a structure to build on.

Inference makes the most of what is in front of it. A model may bring a great deal from its training, but it is still inference over what it holds, and what is new comes from research, from people, and from the work itself. So the rule's work is on the material at hand: taken whole, asked what it really has to say, laid by the gradient.

### 5.2 The step

The tool of the step is the outline: the brief of work not yet done. It says what is to be conveyed, in what order, with what beneath, in simple terms and not in prose, so it is cheap to reason about and cheap to check, and a fault caught there is never written. Talking a thing through before committing to it is the same move, in a session as in a text. And the first step with any material is the same: not to rebuild it under the rule, but to ask what is here, in a session that holds it, and to talk before deciding what to make of it, which is usually an addition to a structure that already stands.
...
What is known to work is to write the whole, since which understandings are biggest is known only against the depth beneath them, then read it under the rule brief by brief, asking of each what it really has to say, in its place among the others and in its prose, and write it again from that. The rounds end when one changes only words.
...
This is what gives the quality, and it is brutally costly. A better rule makes each round shorter, but the cost is in the cascade and the iterations needed and the practice of running it is still in its early days.

### 5.4 The fresh head

A writer cannot see its own text whole, and what it left out it does not know it left out. A session holding only the rule and the draft has no picture of what was meant, so it reads what the text actually gives. It hands back what it did not understand, what came before its ground, and what it would cut. It writes nothing; the writer holds the material and the judgment. It is the peer review an agent gets while it iterates.

### 5.5 The debrief

A session ends, and what it did not write is lost with it. So before it ends it debriefs: the piece it made stands as the brief, and beside it, in a few lines, what it left out, what it left open, and what it decided on its own. Written then or never, since how the work went exists only while the session does. This depends on the work at hand and is not so much an instruction as an exemplification of the rule embodied.

## 6. How far this goes

The rule is for knowledge, and nothing else in it depends on this section. But the shape it found is not about prose, and this is how far it reaches.

Prose is the unbounded substrate of knowledge. It is not special, not specific, not anyone's, and that is why the rule needs no technology and why what is written under it compounds. Nothing like it exists for the rest of what a computer holds. Every program ships its own world, its data shut inside it, and between the worlds people build glue: integrations, exports, adapters, workflows. Unix once had a common floor, and we left it. Where a common substrate would stand there is a market of facades.

AI has arrived into that. A harness is a program like any other, its tools coded one by one, its context assembled for the occasion and thrown away, and the use it is mostly put to is gluing the separate worlds together a little faster. Capability behind a rigid facade, the integrations done by hand. What compounds is the glue, never the parts.

The shape underneath the rule is the answer, taken beyond prose. Wholes that are also parts, laid by the gradient, joined by links, in one medium that everything lives in. If the data programs need to run lived in such a medium the way knowledge does here, a program would say what it takes and what it gives in the medium itself, and programs would meet by matching instead of by glue. What draws a screen would say the same. A model's context would be a selection from the medium, and its output would land back into it. Then programs compound as knowledge does, knowledge compounds more because they do, and the harness dissolves: AI is not integrated with the work, it is in it, in the knowledge, the interactions and the programs alike.

None of this exists, and it is reasoned, not shown. It is the author's aim, pursued in [the openlight project](), and it would be built the way the rule is written: nothing convoluted, a common substrate first class for programs. But first, reform the making of knowledge, with what already exists.
