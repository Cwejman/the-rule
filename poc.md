...
Everything in a space is mounted by its entry, and mounting is a brief's work rather than a list's. Every file and folder beside an entry is named in the entry's prose, and each is named by the brief whose understanding it belongs to, so the order a reader meets the parts in is the order those briefs stand in. A section that gathers the parts into one run of paragraphs mounts a level per paragraph, which is a grain the holarchy does not reach, and it is also the map of the parts to come that [the prose principle](code.md#41-the-prose-of-a-brief) refuses. A link is a different thing and may be written anywhere. *Reasoned, the author's, 2026-09-12, from an entry that listed its five parts in five paragraphs and so never had to ask which was biggest.*
