...
- **Lab 08 is written with the author, not run.** Its edition ([`08-win-driven/code.md`](08-win-driven/code.md)) opens with the why, then what the code changes, then how far it goes; the author writes the surface and the rest of the code from here, and [`comments.md`](08-win-driven/comments.md) holds what the edition still owes. The ontology that was to fill its §4, the code's words and the platform's as one web ordered by win, rested at thirty-one words after eight rounds of fresh readers and was then **set aside by the author (2026-09-07)**: it ran a step ahead of the writing. It stands whole with its evidence in [`08-01-ontology/`](08-win-driven/08-01-ontology/README.md), to be rerun with more ideas once the code is written. What it settled stays cited: one side of a relation is enough when its title reads whole from the other end, the backlink is the interface's to derive, and centrality is a good first guess at dependence order ([`evidence-series.md`](08-win-driven/08-01-ontology/evidence-series.md)). Its commit rule, every change committed and the commit the ratification, caught its first loss the same night it was set: notes left uncommitted were dropped by another session's purification.
