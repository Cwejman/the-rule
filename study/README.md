...
- **Lab 08 is written with the author, not run.** Its edition ([`08-win-driven/code.md`](08-win-driven/code.md)) opens with the why, then what the code changes, then how far it goes, then what knowledge is made of; the author writes the surface and the steward the depth, and [`comments.md`](08-win-driven/comments.md) holds what the edition still owes. Beside it stands the ontology ([`ontology.md`](08-win-driven/ontology.md)), the one home for what the words mean and the current arc of the code's evolution: the code's words and the technical arc's as one web ordered by centrality, with what cutting it as a line showed ([`evidence-series.md`](08-win-driven/evidence-series.md)): one side of a relation is enough when its title reads whole from the other end, the backlink is the interface's to derive, and centrality is a good first guess at dependence order. The ontology runs under a rule of its own, written at its top: every change is committed, and the commit is the ratification; a session commits, the author ratifies, the session ratifies back. Two sessions have cycled under it, and the rule caught its first loss the same night: notes left uncommitted were dropped by another session's purification.
