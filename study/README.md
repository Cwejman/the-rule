...
- **Lab 08 is written with the author, and stands ready to run.** Its edition ([`08-win-driven/code.md`](08-win-driven/code.md)) was written in dialog through 2026-09-09: the why, what it changes, then what knowledge is made of under two laws named in its own brief, the brief holding the contract against the win, the two forces as the code's thesis, why-first, and within it the holarchy and the link, then the ground; five principles, each closing with its strength; how far it goes, moved to the end; and what is open. The markdown practice exists as [`poc.md`](08-win-driven/poc.md), the code worked in files and folders, a first cut reasoned and not yet lived. The narrative principle is minted there, grounded in the reading research with its sources marked unverified, and the run's prompt ([`prompt.md`](08-win-driven/prompt.md)) lifts the no-story constraint the earlier runs carried; it awaits the author's ratification before launch. The ontology that was to fill the edition's fourth section rested at thirty-one words after eight rounds of fresh readers and was **set aside by the author (2026-09-07)**: it ran a step ahead of the writing. It stands whole with its evidence in [`08-01-ontology/`](08-win-driven/08-01-ontology/README.md), to be rerun with more ideas once the code is lived. What it settled stays cited: one side of a relation is enough when its title reads whole from the other end, the backlink is the interface's to derive, and centrality is a good first guess at dependence order ([`evidence-series.md`](08-win-driven/08-01-ontology/evidence-series.md)). The lab's comments file retired once every row had a home; git keeps it.
