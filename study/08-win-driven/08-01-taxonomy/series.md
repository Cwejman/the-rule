...
A team's knowledge compounds only as far as its briefs are safe to stand on. So ground carries a duty: one home per fact, every claim with its confidence, and settled meaning settled for now, given what it stands on.
...
The joining of understandings happens here, across people and sessions, because each piece lands as ground for the next.
...
In the pilot it is a tenet of its own, the field's law, and the spec tree still holds it so. This list folds it in, because a self-governing thing cannot have its storage and its program as two authorities.
...
In the pilot it is its own installed artefact and a tenet of its own. This list folds it in.
...
Where the knowledge arc's holons are [briefs](#brief), the engineering arc's are specifications, and the two merge where both are briefs under one gradient.
...
### Is superseded by [amplify](#amplify)

Kept in this list so its absence is named, and so the amplifier is read against what it replaces.
...
The cut was made by one rule: a relation lives on the side that depends, as a link back. Every relation the ontology stated from both ends, as it was first written in one file, survives on exactly one side; none was dropped. What follows is what changed, and the answer to the question the cut was made to ask. The ontology has since been resolved the same way, in its one file: each relation keeps the one home the series gave it, and the file's own order, centrality, is kept as the gradient. A pass in between gave the ground side a door back, a closing line under each word naming who stands on it; it was cut, because the order already carries the downward direction and the inbound trail is a frontend's to derive, not markdown's to hold.

### Is something lost

Yes, and it is the same thing every time: the earlier word loses its doors downward. Comprehension is the sharpest case. In the web it names the win that counts it, the ground that fills it, the brief that contracts it, the medium where it compounds. In the series it names nothing: three linkless properties and no link at all, because everything reaches it and it reaches nothing. The most central word became the emptiest entry. The same happened to win, which lost the amplifier, the brief and the gradient, and to kernel, which lost substrate and engine and the processor, and now says "both program and storage" in words a reader cannot follow. Centrality in a series is visible only as inbound links, never in the entry itself, so a reader who lands on a central word by link sees the least.

### Is one side enough

For the reader who reads the line, yes, and cleanly. Nothing that comprehension lost is far away: the win is the next heading, and the reader meets each downward relation exactly when its word is given, which is the first law kept to the letter. For the reader who arrives at a word by link, no. They arrive at ground from a brief that stands on it and find no way onward to amplify, to link, to specification, to harness, though all four stand on ground. The web served that reader; the series serves the other. Which reader the document is for decides which form is right, and the code already says which: a brief above holds the point, the link leads back. That is the series' rule, and it is written for the reader who came down the gradient, not the one who arrived sideways.
...
No cold reader has read either file; both verdicts above are the writer's, from inside. The series also has a cost the web does not, which this pass did not measure: a word's relations are now scattered across the entries that stand on it, so the web is the only place to see a word whole, and the series cannot be the sole form. And the rule "the dependent side links" is one rule of two; the other, "the bigger win links down", was not cut, and would give the reverse series. Whether that one reads better is the next lab.
