...
The code is written for prose, and much of it is about prose. The shape underneath is not: wholes that are also parts, arranged topologically so the biggest win comes first. It is the shape everything that let independent work compound has had: small pieces, one shared medium, one simple join. Unix had programs joined by pipes, the web has pages joined by links, and knowledge here has briefs joined by links. Knowledge is the first case rather than the easiest one, because comprehension comes before building, and the rest still has to be built.
