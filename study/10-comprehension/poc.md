...
The making wants to read a document by depth and move a brief without retyping it, and no tool for that is built. Scripts over the headings are enough for now, and this is how they are used. To read by depth, a script prints the headings and the first paragraph under each, to the depth asked for, so a session holds the gradient of a file or a folder without reading it whole; a brief is read in full only where the work needs it. To move a brief, a script cuts it by its heading and places it whole where it goes, and to replace one, the script swaps the brief by its address and touches nothing else. What a session wrote a moment ago it does not read again; the fresh read is the fresh head's job. Seen, in the labs, as the cheapest way to keep a rewrite to the brief named.
