...
It digests only what [the stamp](../poc.md#9-a-file-says-it-is-under-the-code) declares to be knowledge, which is what lets a directory be onboarded a file at a time. So the surface needs no knowledge of harnesses at all: whatever wants to be read writes itself into the substrate first.

What it finds there it does not interpret. What counts as a brief, and what nests beneath it, is [the practice's answer](../poc.md#1-the-medium): a brief is a file or a section in one, a folder is a holon whose brief is its entry, and a level is what stands beneath one brief.
