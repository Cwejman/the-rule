...
*Measured with the surface's check on 2026-09-14; that briefs and their confidence lines grew by addition was seen in the briefs themselves.*
...
*Seen, 2026-09-14: the check was run on the arc that day.*
...
3. The surface: [its entry](surface/README.md), [the lane](surface/lane.md), [the framework](surface/framework.md), [the widgets](surface/widgets.md), [its implementation](surface/implementation.md), and [the refusals](surface/refused.md), which is a flat run looked up rather than read through, so each entry is read for its face and the run is not laid again as a gradient.
...
A stamped file the entry reaches that is missing from this list is added in its place and said so. The order is a claim, and a session that finds a file standing on a later one says so before it moves on.

*Reasoned, 2026-09-14; the faces past the flag in each file that day are in [the record](#8-where-the-work-stands).*
...
The file is read whole, with its line of the check. Then an outline, a few lines per brief and not prose: what the brief gives, the sentence or two its face will be, what goes beneath it, and where one brief is really two. The outline is where a fault is cheapest.
...
A change to the practice, or to a brief other files stand on, can move the ground under files already done. The session reads those files for the change before going on, and brings this plan in line if its own ground moved.

### 4.5 The close

The check is run for the whole arc, since a renumbered heading breaks every link written to it, and the file's line in [the record](#8-where-the-work-stands) is written.
...
## 6. What the author decided

Four decisions were the author's, asked before anything was written, and one more came with the answer. The author decided them on 2026-09-14, and the session acted on them.

*Decided by the author, 2026-09-14.*

### 6.1 The confidence line

A line keeps its grade, its date and its ground in a phrase. Refusals found in a line move to the level's run of refusals, and the path of asks goes to git.

### 6.2 The ideas

The faces past the flag are broken and a paragraph holding several points is split, keeping the author's wording and never restructuring an idea.
...
No trial. The session's own model writes, and every fresh head runs on Opus, which the author judged good enough for the reading.

### 6.5 The commit

One commit at the end rather than one per file, since the author will not ratify the files until they read them whole, and may not have time to. The stamps say in force, not ratified.
...
This is a record, newest first. A session that starts in the middle reads here where the last one stopped.
...
Section 4.2's confidence line, measured once and one level deeper reading shorter while holding more, did not parse for two cold readers, and section 3.3 asks every claim to say its confidence while sections 1, 2, 3, 5 and 6 carry no line.

Three words: section 3.2.2 "the gradients holarchy" wants an apostrophe, section 4.3 "in what grounds" reads as a slip, and section 4.4 "has lived or am able to imagine" wants "is".

*Proposed, 2026-09-14.*

### 8.3 What the pass left open

Whether "grade" for a brief's fold state in the surface and "grade" for a claim's confidence should be one word; the surface's use is older and its code carries it, and the pass left both.
