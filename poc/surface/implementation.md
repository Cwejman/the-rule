...
The process answers six requests: the page itself, the body as JSON, a stream that says when a file under the path has changed, so the client asks for the body again and draws it, the images the body holds, and history's [light tier](history.md#41-the-shallow-tier-is-had-whole) and [heavy tier](history.md#42-the-deep-tier-is-had-a-commit-at-a-time), asked for apart from the body. The body is assembled again whole on any change to its markdown or to an image, which a body this size allows.
...
A sketch is inside the page already, as [markup set into it](#363-a-sketch-is-set-into-the-page). History is written beside the page and never inside it, its light tier as `history.json` and its heavy tier as a file per commit under `commits/`, the paths the live process answers at too. Every other image the body holds as a file is written beside the page, at the path it has in the body, so the page stays light and an image is fetched only when the lane reaches it. Its address carries a hash of its bytes, so a changed image is fetched again rather than kept from before. A remote image stays remote, since it may change after the build, and the page takes its new shape if it has.
