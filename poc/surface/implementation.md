...
The focus is found on every scroll: the article under the reading line, or the nearest above it. The line stands on the opening at the top of the lane and eases to the middle of the viewport over the first third of a screen of scrolling, and a move to a brief finds the scroll that brings the brief to the line by halving, since the line moves with the scroll. The room above the lane is set so the opening's heading stands level with the first cell of the shape. When it changes, the page's address is replaced without entering the history, and the widgets that depend on the focus draw again.
