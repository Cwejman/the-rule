...
Backspace folds the brief above as well. [On the map it is one act](canvas.md#52-the-keys-act-where-the-hand-is-and-the-moves-are-each-panes-own), asking for less of whatever the reader is inside, and folding the brief above is what that means in the prose.

The arrow keys move the focus and fold nothing: up and down to the brief before and after in the lane, left to the parent, right into the first brief beneath when it stands in the lane. Shift with up and down moves to the brief before or after at this level instead, passing over whatever stands beneath it, [as it does on the map](canvas.md#52-the-keys-act-where-the-hand-is-and-the-moves-are-each-panes-own). Shift with left and right unfolds the scope one level less or one level more, as [the depth strip](canvas.md#61-the-depth-strip) does. Enter and shift with enter [scope the lane](#7-scoping-the-lane).
