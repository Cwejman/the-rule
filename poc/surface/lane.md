...
The acts follow the highlight as well: while a brief is pointed at, the key on its line is inked and the key on the focus's line goes back to the pointer's cap, and the space bar folds the brief under the pointer rather than the one on the reading line.

*Reasoned, the author's, 2026-09-13; the still pointer after a scroll, 2026-09-14; the acts following the pointer, reported by the author and built 2026-09-17.*
...
The prose fades out a little in from the top and the bottom of the viewport, over a short run, so [the strip](framework.md#3-the-strip) at the foot never reads as an edge of the text.
...
Backspace folds the brief above as well. [On the map it is one act](canvas.md#52-the-keys-belong-to-the-map-while-it-stands-and-the-moves-are-each-panes-own), asking for less of whatever the reader is inside, and folding the brief above is what that means in the prose.

The arrow keys move the focus and fold nothing: up and down to the brief before and after in the lane, left to the parent, right into the first brief beneath when it stands in the lane. Shift with up and down moves to the brief before or after at this level instead, passing over whatever stands beneath it, [as it does on the map](canvas.md#52-the-keys-belong-to-the-map-while-it-stands-and-the-moves-are-each-panes-own). Shift with left and right unfolds the scope one level less or one level more, as [the depth strip](canvas.md#61-the-depth-strip) does. Enter and shift with enter [scope the lane](#7-scoping-the-lane).
...
How here is placed is the run of levels from the root down to [the brief in focus](#4-where-you-are-is-the-brief-in-focus), not only to the scope: a reader who has scrolled into a brief has its heading off the screen, and where they stand is what the line is for.

The run says three things, and the type tells them apart rather than any shape drawn around them. The scope, the way down to the reading the reader has opened, stands bold. The address within it stands at normal weight and fainter, since it is a place inside what the bold has already named. The head, what the reader is looking at, takes full ink, and where it opens onto a reading of its own it is underlined in its branch's hue, as a link that can be followed is. Each is lit in its colour when pointed at; a name above the scope is a press that scopes out to it, and one within the scope a press that goes there.

The acts of the rightmost stand after the run. Where it is a reading of its own, [the keys](widgets.md#17-a-key-stands-as-a-badge-where-its-act-stands) for entering it and for widening out of it both stand there, since that is what enter and shift with enter do where the reader is standing; where it is a section, only widening does.

Where the line is too narrow to read them all, the run is cut at its root until every name it still shows reads whole, with one mark standing for what was cut, since a name squeezed to a letter says nothing; the brief in focus is never cut. The prose is always clear beneath it, whatever the fade at the top is doing, and a wing's figures begin below it too.
