...
How here is placed is the run of levels from the root down to [the brief in focus](#4-where-you-are-is-the-brief-in-focus), not only to the scope: a reader who has scrolled into a brief has its heading off the screen, and where they stand is what the line is for. The names stand faint with a small chevron between, each lit in its branch's colour when pointed at; the scope root a step darker, and the brief in focus darker still, since that is where the reader is. A name above the scope is a press that scopes out to it, and one within the scope a press that goes there. Where the line is too narrow to read them all, the run is cut at its root until every name it still shows reads whole, with one mark standing for what was cut, since a name squeezed to a letter says nothing; the brief in focus is never cut. At the run's right stands [the key that widens the scope](widgets.md#17-a-key-stands-as-a-badge-where-its-act-stands) by a level. The prose is always clear beneath it, whatever the fade at the top is doing, and a wing's figures begin below it too.
...
*Reasoned, the author's, 2026-09-14; the trail as cells the author's ask of 2026-09-15, titles having been tried first that day; the run reaching down to the focus rather than to the scope, his ask of 2026-09-16, from reading on a phone and finding he wanted the address of where he had scrolled to.*
