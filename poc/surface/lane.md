...
How here is placed is the run of levels from the root to the scope. The levels above stand as faint names, each a press that scopes out to it, the scope root a step darker, and a small chevron between, each name lit in its branch's colour when pointed at. At its right stands [the key that widens the scope](widgets.md#17-a-key-stands-as-a-badge-where-its-act-stands) by a level, since the run is what that act moves. The prose is always clear beneath it, whatever the fade at the top is doing, and a wing's figures begin below it too.
