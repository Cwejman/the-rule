...
History is how the knowledge came to be what it is where a reader stands: which commits touched it, and the stories told over them. Git's history is read as a holarchy of its own, with the stories as its briefs and the commits as the leaves beneath them, and the canvas and the lane draw it as they draw any reading, opened apart from the body and never placed in it.

Its data is built; its reading and its view are not. What stands beneath is the design settled in talk, and the data built to answer what the view will ask of it.
...
A story is a brief of the history's own file, a heading and its prose, and a larger story holds smaller ones as briefs beneath it, as any brief holds its level. A story names its commits as [a lone link places a part](../practice.md#3-a-lone-link-places-a-part): a paragraph that is nothing but a link to a commit, or to a run of them, stands the commits there as leaves.
...
A commit leaf is a brief like any other in what the surface holds of it: an address, which is the story's with the commit's hash beneath it, a title, which is its subject, and a parent, the story that placed it. What it carries instead of prose is the briefs it touched and a way to the lines it changed.

A brief it touched is named by its path within its file, from the file's own title down, since where a file is placed in the body changes and the commit does not; the page lays that path onto the body when it draws.
...
### 2.5 It stands at the root of the repository, and is never placed

The stories live at the root of the repository, as `history.md` first, and as a folder `history/` once one reading no longer holds them, as any file becomes one. Nothing places them, and the surface knows the history by where it stands: it is traced only when the history is opened, never into the body, and the check does not list it as an orphan.
...
What a view asks decides what is fetched. The canvas, the colouring and the filter by address need only which commits exist and what each touched; only a change shown in place needs the lines. So the data comes in two tiers: a light one had whole, and a heavy one had a commit at a time, and neither is ever fetched once per commit to draw a picture.
...
### 4.1 The light tier is had whole
...
The file is `history.json`, data beside the page, which shares a name with the stories' `history.md` and nothing else. The root is the path the surface is handed, and each root keeps its own. A commit is read once, kept by its hash in the repository's git folder, and never read again. It stands beside the page and never inside it, since it weighs a quarter of the page.
...
### 4.2 The heavy tier is had a commit at a time
...
*In force, from [the history of the first attempt](../../ideas/history.md#2-why-it-came-out); the author's rule since 2026-09-17. Measured around both tiers, 2026-09-21: `/body` stood at 0.71 to 0.75 seconds throughout, and the page's script did not change by a byte.*
...
That is what [a commit of one purpose](../practice.md#13-a-commit-is-one-purpose) is for: it is met where its work is.
...
A commit's figure draws a bar for every reading it touched, each in that reading's branch hue. A local commit is one bar, and one that reached many readings a row of hues, whether one purpose propagated or several were conflated; the story over it says which. Filtered by an address, it stands in every reading it touched.
...
### 5.2 A selection shows what it touched
...
### 5.3 History is drawn top-down, as any reading is
