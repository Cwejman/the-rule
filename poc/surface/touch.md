...
A finger is a fatter pointer, and everything it presses is given room it can find, near the forty-four pixels a hand asks for. That is not a coat of paint over the same design: where the room cannot be had, the thing itself changes, and the briefs beneath say where.

A press too small to hit is worse than no press at all, since the reader cannot tell a miss from a refusal.

*Reasoned, from the author's ask of 2026-09-16 that a press have real estate; as built the same day.*
...
*Reasoned, 2026-09-16, from [the room a finger asks](#2-room-for-a-finger); as built the same day.*
...
Whether the shape or the outline is the first thing a reader reaches for on a phone, since the outline carries names and the shape does not.

The flick is answered by the platform: it reads a wheel, and a finger sends none, so it cannot fire on touch whatever the setting says. The setting is left standing, and that it says nothing on a phone is not yet said anywhere a reader would look.

[Pulling past the top](lane.md#74-pull-past-the-top) reads the wheel as well, so it is not built for a finger, and a reader widens the scope from the names in the way down instead.
